 ...
 */
@TeleOp
public class A_2ControlSirachaV2 extends OpMode {
    ...
    public void loop() {
        ...
        if (horizdelay != 0) {
            if (System.nanoTime() > 6e+8 + horizdelay) {
                horizdelay = 0;
                horizPID = true;
        ...
        if(currentliftheight > 200){
            liftdistworking = false;
        }
        ...
        if(horizpos < 600){
            ...
            if(nonPIDliftControl == false && liftTargetPos < 100 && liftdistworking){
            ...
            if (horizpos < 100 && horizTargetPos < 100 && nonPIDhorizControl == false){
                horizPID = false;
                horizontal_slide.setPower(0);
            }
        }
